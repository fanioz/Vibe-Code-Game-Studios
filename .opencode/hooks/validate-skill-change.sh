...
# Only act on skill files (.claude/skills/ or .claude/skills/)
if ! echo "$FILE_PATH" | grep -qE '(^|/)\.(opencode|claude)/skills/'; then
    exit 0
fi

# Extract skill name from path (.claude/skills/[skill-name]/SKILL.md)
SKILL_NAME=$(echo "$FILE_PATH" | grep -oE '\.(opencode|claude)/skills/[^/]+' | sed 's|\.[a-z]*/skills/||')
