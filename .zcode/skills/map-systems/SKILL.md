...
**After dependency mapping is approved, spawn `technical-director` via Task using gate TD-SYSTEM-BOUNDARY (`.zcode/docs/director-gates.md`) before proceeding to priority assignment.**
...
**After priorities are approved, spawn `producer` via Task using gate PR-SCOPE (`.zcode/docs/director-gates.md`) before writing the index.**
...
Using the template at `.zcode/docs/templates/systems-index.md`, populate the
...
**After the systems index is written, spawn `creative-director` via Task using gate CD-SYSTEMS (`.zcode/docs/director-gates.md`).**
