...
- **`creative-director`** — gate **CD-PILLARS** (`.zcode/docs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (`.zcode/docs/director-gates.md`)
...
**After identifying biggest technical risks, spawn `technical-director` via Task using gate TD-FEASIBILITY (`.zcode/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via Task using gate PR-SCOPE (`.zcode/docs/director-gates.md`).**
...
4. **Generate the game concept document** using the template at
   `.zcode/docs/templates/game-concept.md`. Fill in ALL sections from the
...
If yes, generate the document using the template at `.zcode/docs/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
